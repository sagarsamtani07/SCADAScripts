...
api_key = api_key_eric

api = shodan.Shodan(api_key)
cidr_list = ['192.35.79.0/24', '66.162.22.128/25', '162.129.0.0/16', '129.176.0.0/16', '143.111.0.0/16']
...
for cidr in cidr_list:
    ...
    print("cidr: {0}, {1} records in Shodan".format(cidr, total))
    for i in range(total // 100 + 1):
        result_list = api.search('net:' + cidr, page=i+1)
        for d in result_list['matches']:
            try:
                counter += 1
                ...
                log.write("IntegrityError: {0}, at counter {1}\n".format(err, counter))
                log.flush()
            except Exception as err:
                log.write("Unsolved exception: {0}, at counter {1}\n".format(err, counter))
                log.flush()
