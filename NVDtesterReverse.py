        ...
        with nvddb.cursor() as cursornv:
                ...
                for r in result2:
                        ...
                        for r in result:
                            ...
                            v = re.search("([^(0-9)][^(\.)])(%s)([^0-9])"%version, data)

                            newprod = str_replaceprod(product)
                            p = re.search("(%s)"%newprod, data)

                            if p and version == "":
                                try:
                                    with vulnerablesystems.cursor() as cursorvs:
                                        # Create a new record in Vulnerable systems
                                        sql = "INSERT INTO `nvdvulntest` (`ShodanID`, `ipaddr`, `CVE-ID`, `vendor`,`product`,`version`, `score`) VALUES (%s, %s, %s, %s, %s, %s, %s)"
