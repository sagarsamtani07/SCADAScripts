...
record_num_scada = 200000
record_num_non = 200000

# the skip_factor is used for hold-out sampling, separating the dataset into 2/3 and 1/3
# setting skip_factor to 0 or stratified_sampling to False will prevent this
# db_suffix is used to refer to db tables
stratified_sampling = True
skip_factor = 3
db_suffix = "_2_3" if stratified_sampling else ""

# this list contains 3 empty dicts for overall n-gram stats: [0]: gram[0] (aka unigram), [1]: gram[1] (bigram), [2]: gram[2] (trigram)
...
# shodan.scadashodan is the original table containing annotated SCADA devices by Eric
# later shodan.sy_sfs_scadashodan was brought out as an additional SCADA device set, but seemed to be of lower quality
sql1 = """
SELECT DISTINCT portnum
FROM shodan.scadashodan
UNION
SELECT DISTINCT port
FROM shodan.sy_sfs_scadashodan
"""
cur.execute(sql1)
...
# shodan.nonscadashodan and shodan.nonscadashodanmore are the two tables containing non-SCADA device records.
sql1b = """
SELECT DISTINCT portnum
...
# ================ FOR SCADA DEVICES =================
# select the first record_num_scada records from scadashodan: port number, banner data
sql2 = """
...
for row in cur:
    ...
    # you can modify "counter % skip_factor != 0" to "counter % skip_factor == 0" to get 1/3 or 2/3 of the dataset
...
port_count_sql = """
INSERT INTO sy_device_port_count_new (port, scada, nonscada)
VALUES ('%s', '%s', '%s')
"""

ngrams_sql = """
INSERT INTO sy_scada_port_ngrams_new (port, type, word, count)
VALUES ('%s', '%s', '%s', '%s')
""".format(db_suffix)

ngrams_non_sql = """
INSERT INTO sy_nonscada_port_ngrams_new (port, type, word, count)
VALUES ('%s', '%s', '%s', '%s')
""".format(db_suffix)
