...
with open("C:/Users/ejgross/Desktop/ShodanRepo/2014-12-23.json", "rb") as src:
    with open("C:/Users/ejgross/Desktop/log.txt", "w+") as log:
        for s in src:
            ...
            # sample the first 1,000,000 record
            if counter > 10000000:
                exit(0)

            # used to systematic sampling to get 50,000 records
            if counter % 200 != 0:
                continue

            print("Record {0} selected".format(counter))

            # Reconnecting to the database every 200,000 records
            if counter % 200000 == 0:
                cur.close()
                db.close()
                db = pymysql.connect(host="128.196.27.147",  # your host, usually localhost
